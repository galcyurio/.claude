...
1. **간결함 우선**: 개요는 **1~3문장**, 작업사항은 **2~4 bullet 이내**로 짧게 쓴다. 그 이상 쓰지 않는다.
2. **작업사항은 구체적 구현을 쓰지 않는다**: 함수명/클래스명/파일 경로/조건 분기/동작 힌트 같은 세부는 작업사항에 넣지 않는다. 리뷰어는 코드 diff로 본다. 작업사항은 "무엇이 달라졌는지"를 1~2단어 수준으로만 표현한다.
3. **커밋 메시지와 중복 금지**: 커밋 제목/메시지를 PR 본문에 나열하지 않는다. 리뷰어는 커밋 목록을 별도로 본다. PR 본문은 "커밋 히스토리로는 보이지 않는 맥락"만 담는다.
4. **리뷰어 관점으로 요약**: "왜 이 PR이 필요한가"와 "리뷰어가 무엇에 집중해야 하는가"만 적는다. 세부 구현/변수명/함수 동작은 코드에서 보므로 본문에서 제외한다.
5. **구어체(존댓말)로 작성**: PR 본문 문장은 `~합니다` 체로 작성한다. `~한다` 같은 평서형 문어체는 사용하지 않는다.
   - 예: `~ 문제를 해결한다.` → `~ 문제를 해결합니다.`
   - Bullet 항목이 체언으로 끝나는 경우(예: `~ 추가`, `~ 처리`)는 그대로 둔다. 서술형 문장으로 끝나는 경우에만 존댓말을 적용한다.
6. **작업사항은 가능한 한 mermaid로 표현한다**: 화면 전환, 호출/데이터 흐름, 모듈/클래스 구조 변경이 조금이라도 있으면 bullet 대신 mermaid 다이어그램을 사용한다. 단순 변경(버그 픽스, 단일 함수 수정, 문구/스타일 변경)에만 bullet만 쓴다. 애매하면 mermaid를 선택한다.
...
위 유형 중 하나라도 해당되면 **기본적으로 mermaid를 사용**한다. bullet만 쓰는 경우는 단순 변경(버그 픽스, 단일 함수 수정, 문구/스타일 변경 등)으로 제한한다.
