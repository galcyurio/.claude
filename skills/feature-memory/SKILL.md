...
- **Figma**: 분류된 Figma URL 각각을 fileKey+nodeId로 파싱. 페이지 단위 nodeId는 `get_design_context`가 항상 실패(`선택된 레이어 없음`)하므로 **frame 단위로 우회**한다.
  ...
  **1단계 — frame 목록 추출**: `mcp__claude_ai_Figma__get_metadata(fileKey, page_nodeId)` 호출. 응답은 수십만 자로 token limit 초과하지만 도구가 자동으로 임시 파일에 저장한다 (응답 메시지에 파일 경로 포함). 그 임시 파일에서 다음 명령으로 직계 자식 frame만 추출:
  ...
  각 줄은 `nodeId|name|WxH` 형태. 이걸 frame 목록 표로 정리하고 본문 `## 📚 Reference`의 `Figma frame 목록` toggle에 부착. 각 frame은 `https://www.figma.com/design/{fileKey}/?node-id={nodeId_with_dash}` (콜론을 대시로 변환) 형식의 클릭 링크.
  ...
  **2단계 — frame별 디자인 컨텍스트 incremental fetch (hash diff)**: Figma API는 `last_modified`를 제공하지 않으므로 **응답 코드 hash 비교 방식**으로 변경 감지를 구현한다.
  ...
  4. 각 frame에 대해 hash 비교:
     ...
     - hash 동일 → 본문에 반영하지 않음
  5. 1단계에서 더 이상 보이지 않는 frame (직전 dict에는 있는데 새 목록에 없음) → `## 변경 이력`에 `- (YYYY-MM-DD) · [Figma](url) · frame 삭제 — "{name}"` prepend.
  ...
  **3단계 — 변경된 frame의 디자인 컨텍스트를 Reference에 보존**: 위에서 `신규/변경`으로 식별된 frame들의 응답 코드(asset URL 정리 전)를 본문 `## 📚 Reference`의 `Figma 디자인 컨텍스트 (최근 변경)` toggle에 저장. 형식:
  ...
  **비용 제어**: frame 수가 30개 초과면 1회 fetch만으로도 부담이 크니 `figma_frame_hashes`가 `__skip__`으로 명시 설정된 경우 fetch 자체를 skip하고 frame 목록만 갱신한다 (사용자가 비용 감수 불가 시 활용).
...
| `figma_frame_hashes` | rich text | Figma frame별 디자인 컨텍스트 hash JSON. `{"488:2586": "sha256...", ...}` 형식. STEP 2.3 Figma 2단계의 incremental 변경 감지용. `__skip__`으로 설정하면 frame fetch 자체를 skip. 빈 값(`""`)이면 첫 갱신 시 `{}`로 초기화 |
