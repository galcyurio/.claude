...
│  STEP 3: markdown 본문 조합 (5 섹션)                          │
│  STEP 4: Notion 페이지 본문 블록 전체 교체                    │
...
>
> **체크박스 보존 알고리즘** (STEP 4의 단순화):
> 1. fetch한 본문에서 `## 🎯 내가 해야 할 일`·`## ⚠️ 영향 주는 변경` 섹션을 파싱
> 2. **각 항목별로 체크 상태(`- [ ]` vs `- [x]`)와 본문 텍스트를 dict에 저장** (key = 항목 제목 정규화)
> 3. 새 본문을 만들 때, 같은 키가 dict에 있으면:
>    - 체크돼 있었으면 → ✅ 완료 섹션으로 이동 (재출현 금지)
>    - 미체크였으면 → 그대로 미체크 유지 (본문 정보는 갱신 가능, 체크 상태는 불변)
> 4. dict에 없는 신규 항목만 새 미체크(`- [ ]`)로 추가
>
...
3. **본인 식별 fetch (병렬, 한 세션 1회로 캐싱)**:
   - `mcp__claude_ai_Atlassian__atlassianUserInfo()` → `self.jira_account_id`, `self.display_name`, `self.email`
   - `mcp__claude_ai_Slack__slack_search_users(query=self.email)` → `self.slack_user_id`, `self.slack_display_name`
   - `Bash(gh api user --jq '.login')` → `self.github_login`
...
- **Figma**: 분류된 Figma URL 각각을 fileKey+nodeId로 파싱.

  **1단계 — 디자인 컨텍스트**: `mcp__claude_ai_Figma__get_design_context(fileKey, nodeId)` 호출. 토큰 초과/"선택된 레이어 없음" 등 실패 시 → `mcp__claude_ai_Figma__get_screenshot`으로 폴백 (스크린샷 1장 부착, URL은 단기 유효이므로 본문에 직접 임베드하지 말고 `## 데이터 소스 상태`에 성공 표기만).

  **2단계 — 페이지 단위 frame 목록 추출** (nodeId가 페이지 단위인 경우에만): `mcp__claude_ai_Figma__get_metadata(fileKey, nodeId)` 호출. **응답은 수십만 자로 token limit 초과하지만 도구가 자동으로 임시 파일에 저장한다** (응답 메시지에 파일 경로 포함). 그 임시 파일에서 다음 명령으로 직계 자식 frame만 추출:
  ...
  각 줄은 `nodeId|name|WxH` 형태. 이걸 표로 정리해 본문 `## 📚 Reference`의 `Figma frame 목록` toggle에 부착. 각 frame은 `https://www.figma.com/design/{fileKey}/?node-id={nodeId_with_dash}` (콜론을 대시로 변환) 형식의 클릭 링크.

  **모든 단계 실패** 시 `## 한눈에 보기`의 Figma 링크만 유지하고 `## 데이터 소스 상태`에 ⚠️ 1줄 기록.
...
**A. 내가 해야 할 일 (TODO)**
- Jira에서 본인이 `assignee`인 Epic·subtask (status가 Done 아님). `assignee.accountId == self.jira_account_id`로 판정
...
- 마감 있는 작업 (예: QA 공유 예정일이 본인 담당)
- 본인이 결정/실행해야 할 미해결 항목

**B. 나에게 영향 주는 변경 (FYI)**
- API 명세 변경 (Notion API 문서, Jira 코멘트 등 **결정 사항으로 기록된 변경**) — 사용자 역할이 클라이언트면 영향
- 기획 변경 중 사용자 작업에 영향 주는 결정 (예: UI 변경 → Android 영향)
- 다른 사람이 **확정한** 사항 중 사용자가 알아야 할 것 (예: A/B 노출 범위, recommendation_type enum 추가)
- 새 의존성·외부 요구사항
- **단, 단순 Slack 논의/질문은 본인 명시 멘션 + 책임 게이트 통과 시에만 포함** (아래 룰 참조). 멘션 없는 Slack 논의는 Reference로.
...
## STEP 4: Notion 페이지 본문 업데이트 + 체크박스 상태 보존

> **사전 조건**: STEP 2.1에서 본문 fetch가 이미 끝나 있어야 한다. fetch 결과가 없으면 STEP 4 진입 금지(= replace_content 호출 금지). 사용자가 페이지에서 한 체크가 사라지는 근본 원인이 이 단계 누락이다.

1. STEP 2.1에서 받은 본문 markdown을 그대로 사용한다 (재fetch 불필요).

2. **체크박스 상태 파싱 (핵심 멱등성)**: 본문의 `## 🎯 내가 해야 할 일`, `## ⚠️ 영향 주는 변경` 섹션에서 모든 항목과 체크 상태(`- [ ]` 또는 `- [x]`)를 추출. 항목명(우선순위 마커 제외한 굵은 텍스트와 한 줄 설명)을 정규화된 키로 사용.
   - **검증**: 파싱 결과 `checked_items` / `unchecked_items` 두 리스트를 만들고, 로그/메모에 카운트를 남겨 사용자가 검수할 수 있게 한다 (예: "기존 본문에서 체크 N건, 미체크 M건 추출").

3. **완료된 항목 archive 처리**:
   - 위 두 섹션에서 `- [x]` 체크된 항목 → `## ✅ 완료된 항목 (자동 보관)` 섹션으로 이동. 형식: `- ✅ (YYYY-MM-DD에 체크됨) [TODO|FYI] **{항목명}** — {원본 설명}. [Slack](url)` (도메인 라벨 인라인 링크)
   - 완료 시각은 본 갱신 시각을 사용
   - 기존 "완료된 항목" 섹션 내용은 보존하고 새 완료 항목을 위에 prepend

4. **신규 후보 dedup**: STEP 2.5에서 추출한 새 후보 리스트를 기존 미체크 항목 + 완료된 항목과 비교. 항목명 정규화 키가 같으면 중복으로 간주.
   - 미체크에 이미 있으면: 그대로 유지
   - 완료된 항목에 이미 있으면: 다시 추가 안 함
   - 둘 다 없으면: 새 미체크(`- [ ]`)로 추가

5. **변경 이력 보존**: 기존 본문에서 `## 변경 이력` 섹션 파싱 → STEP 3 신규 항목과 합침 (신규가 위, 기존이 아래). 항목 형식은 `[Slack](url)` 같은 도메인 라벨 인라인 링크.

6. **본문 블록 일괄 교체**: `notion-update-page` command `replace_content`로 페이지 본문 전체를 새 markdown으로 덮어쓰기. 체크박스는 `- [ ]` / `- [x]` 형식 그대로 유지.

7. **이전 inline comment 정리 (1회성 마이그레이션)**: 만약 페이지에 이전 명세로 부착된 `🔗 출처:` 마커 자동 생성 댓글이 있다면 `resolved` 처리(또는 사람이 페이지에서 직접 정리). 새 본문에서는 댓글 부착 단계 자체가 없음.

8. **Rate limit 처리**: 429면 `Retry-After`만큼 대기 후 1회 재시도.
...
| Figma | 파일 삭제, 권한 만료, 잘못된 fileKey/nodeId, 토큰 초과 | 1단계: `get_design_context` 실패 시 `get_screenshot` 폴백. 2단계(페이지 단위 nodeId): `get_metadata` 응답의 토큰 초과는 **정상 동작** — 도구가 만든 임시 파일에서 jq + grep으로 frame 목록 추출. 모든 단계 실패 시 ⚠️ + `last_error` 기록 + Slack 알림 |
...
- **본문은 매 실행마다 전체 재생성**한다. 사람이 페이지 본문(`## 한눈에 보기`~`## 데이터 소스 상태`)을 손으로 편집해도 다음 실행에서 덮어씌워진다. 자유 메모가 필요하면 child page를 직접 만들어 사용한다.
