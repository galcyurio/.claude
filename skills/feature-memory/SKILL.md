...
| `register <epic-key> [...옵션]` | 새 피처를 등록한다 (Notion DB에 빈 페이지 생성, `status = active`) |
...
사용법:
  ...
  /feature-memory register HDA-12345 [--alias <name>...]          새 피처 등록
...
`/feature-memory register HDA-12345 [--alias <name>...]`
...
4. Notion DB에 새 페이지 생성 (`mcp__claude_ai_Notion__notion-create-pages`):
   ...
   - properties:
     - `epic_key` = `HDA-12345`
     - `aliases` = `--alias` 값들 (없으면 빈 multi-select) — URL 아닌 자유 태그
...
| `figma.com/design/...`, `figma.com/board/...` | **Figma** | `get_metadata` → 변경 시 `get_screenshot` |
...
- **Figma**: 분류된 Figma URL 각각을 fileKey+nodeId로 파싱 → `mcp__claude_ai_Figma__get_metadata` 병렬 호출. `last_modified >= since`면 `mcp__claude_ai_Figma__get_screenshot` 추가
...
| `aliases` | multi-select | 한국어 별칭 / 자유 태그 (URL 아님) |
...
| Figma | 파일 삭제, 권한 만료, 잘못된 fileKey/nodeId | 동일 (썸네일 캡처 실패는 메타만으로 진행) |
...
# 별칭만 추가
/feature-memory register HDA-12345 --alias "로그인 개선"
