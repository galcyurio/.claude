...
│  STEP 2: 6 소스 병렬 수집  ┌─ Jira (Epic + 하위 + 코멘트)    │
...
> **⛔ API 문서는 1급 참고 문서다.** API 문서는 이 피처의 **서버-클라 계약**이므로, Notion(`notion.so`)에 있든 외부 호스트(`docs.prnd.co.kr` 등)에 있든 **항상 별도 소스로 인식·수집·인용**한다. "Notion"으로 뭉뚱그리거나 "Extra"로 흘려보내지 않는다. 데이터 소스 상태에도 **별도 라인**으로 표기하며, 인식된 API 문서 URL이 0개여도 `⚪ API 문서 (링크 없음)`으로 명시한다.

**SSOT 원칙**: URL은 모두 Jira 이슈에서 추출한다(원본 SSOT). 추출한 핵심 소스 URL(기획서·API 문서·Figma·Slack)은 DB 가시성을 위해 동명 URL property에 미러한다(파생 캐시 — STEP 5에서 기록). 누락된 URL이 있으면 Jira 이슈 본문/코멘트/웹 링크에 추가하여 다음 갱신에 자동 반영. **API 문서 URL이 Jira에 없으면** 데이터 소스 상태에 `⚪ API 문서 (링크 없음 — Jira에 추가 시 자동 반영)`로 남기고 `API 문서` property는 공란으로 둔다.
...
9. `## 📚 Reference` (Notion `<details>` toggle 또는 명시 구분선 아래, 평소엔 안 봄)
   - 기획 내용 (Notion 기획서 인용)
   - API 명세 (API 문서 전문 인용 — Notion·외부 모두, 1급 계약)
...
- 6 소스의 `since` 이후 활동을 **시간 역순**으로 최대 N=20개
- 항목 형식 (1줄):
...
- ✅ Notion 기획서 (최근 last_edited YYYY-MM-DD)
- ✅ API 문서 (엔드포인트 N개, last_edited YYYY-MM-DD) — 없으면 `⚪ API 문서 (링크 없음 — Jira에 API 문서 URL 추가 시 자동 반영)`
...
<details>
<summary>API 명세 전체 (1급 계약 — 변경 시 ⚠️로 격상)</summary>

각 엔드포인트별 query_params·response·시나리오. 공통 파라미터, 로그 수집 명세 포함. **API 문서는 서버-클라 계약이므로 변경 없어도 전문을 항상 유지**하고, 출처 링크(Notion/외부)를 함께 표기한다.
</details>
...
> **항상 실행**: 6 소스 신규 0건이어도 이 단계는 건너뛰지 않는다 (체크박스 sweep 때문). `last_run_at`만 갱신하고 종료 금지.
...
- **⛔ 위 목록에 없는 섹션은 사람이 관리하는 섹션이므로 절대 건드리지 않는다.** 사용자가 직접 추가한 `## 🛠 Android 구현 계획` 같은 섹션이 여기 해당한다. 내용·위치를 그대로 보존하고, 자동 생성 항목을 그 안에 끼워 넣지도 않는다. 관련 내용을 반영해야 하면 🎯/⚠️에 항목을 만들고 본문에서 그 섹션을 참조만 한다.
...
> **SSOT 원칙**: 외부 링크의 **원본(SSOT)은 Jira 이슈**다. 단, 자주 보는 핵심 소스(기획서·API 문서·Figma·Slack)는 **DB 가시성을 위해 URL property로 미러**한다 — 매 갱신 시 Jira에서 추출한 값으로 덮어쓰는 **파생 캐시**이며, 원본 추가/수정은 Jira에서 한다. 그 외 URL은 property에 두지 않는다.
