...
6. **모드 분기 처리**
    ...
    - `parent-with-subtasks` 모드:
      ...
      - **Why:** 형제 type은 부모 에픽의 자식 다수 type을 따라야 한다(메모리: `feedback_jira_issue_type_match_epic_standard`). 한국 안드로이드 프로젝트(HDA) 에픽 표준은 `작업` type이다. `하위 작업`은 일부 시리즈에서만 사용되며 default가 아니다.
...
7. **설명 정리**
    ...
    - `single` 모드와 `parent-with-subtasks` 모드의 하위 이슈 설명 모두 최소 아래 항목을 포함한다. 2단계 코드/파일 분석 결과가 있어도 구현 방법은 옮기지 않고 *무엇을·왜*만 반영한다:
      - `배경·문제` — 어떤 문제·불편·필요가 있는지, 왜 이 작업이 필요한지
...
```json
{
  ...
  "content": [
    ...
    {
      "type": "paragraph",
      "content": [{ "type": "text", "text": "어떤 문제·불편·필요가 있는지, 왜 이 작업이 필요한지" }]
    },
    {
      "type": "heading",
      "attrs": { "level": 3 },
