...
- **이슈 제목**: 예) `로그인 기능 추가`
- **App 필드 값**: 예) `고객`, `라이브러리`
- **하위 이슈 목록(선택)**: `parent-with-subtasks` 모드에서만 사용하며, 각 항목은 `제목`, `배경·문제`, `목표`를 포함
...
2. **브랜치 이슈 키 추출 + 코드 분석 (병렬)**
   ...
   **코드/파일 분석:**
   사용자가 선택한 코드나 파일이 있다면, 그 코드가 다루려는 **무엇을·왜**(배경·문제, 목표)를 파악해 description을 작성한다.
   - 코드 분석 결과(파일 경로, 클래스/함수명, 기술적 고려사항 등 구현 방법)는 description에 옮기지 않는다. *무엇을·왜*를 이해하기 위한 보조로만 쓴다.
...
6. **모드 분기 처리**
    - `single` 모드:
      - 5단계에서 생성한 상위 이슈 1개만 유지한다.
      - 하위 이슈 생성 단계는 건너뛴다.
    - `parent-with-subtasks` 모드:
      - 각 하위 이슈를 `--type "작업" --parent <상위키>`로 생성한다. type은 `하위 작업`이 아니라 `작업`을 사용한다.
      - 기본 생성 명령:
      - `acli jira workitem create --project HDA --type "작업" --summary "..." --parent HDA-XXXX --json`
      - 하위 이슈 summary는 `<하위 제목>`으로만 생성한다(상위 제목 결합은 Jira Automation이 처리).
      - **Why:** 형제 type은 부모 에픽의 자식 다수 type을 따라야 한다. 한국 안드로이드 프로젝트(HDA) 에픽 표준은 `작업` type이다. `하위 작업`은 일부 시리즈에서만 사용되며 default가 아니다.

7. **설명 정리**
    - **구현 방법·접근(어떤 파일/함수/기술을 어떻게 바꿀지)은 적지 않는다.** 접근은 구현 중 바뀔 수 있으므로, 설명에는 *무엇을·왜*(배경·문제, 목표)만 기술한다.
    - 제목 prefix(`[App]`)는 Jira Automation이 자동 추가하므로 수동으로 붙이지 않는다.
    - summary는 prefix 없이 순수 제목만 입력한다. (예: `로그인 기능 추가`)
    - `single` 모드와 `parent-with-subtasks` 모드의 하위 이슈 설명 모두 최소 아래 항목을 포함한다. 2단계 코드/파일 분석 결과가 있어도 구현 방법은 옮기지 않고 *무엇을·왜*만 반영한다:
      - `배경·문제` — 이 작업이 필요해진 경위. **기존 동작에 결함이 확인된 경우에만 문제로 서술한다.** 아직 구현되지 않은 기능을 새로 만드는 작업이라면 없는 결함을 지어내지 말고, 무엇이 새로 필요해졌는지를 그대로 적는다. `~할 방법이 없다`, `~을 확인할 수 없다` 같은 결핍 프레이밍은 결함이 아니라 미구현 상태를 문제처럼 보이게 만들므로 쓰지 않는다.
        - 신규 구현 (O): `적립금 화면에 현재 적립금과 소멸 예정일을 노출하기로 했다.`
        - 신규 구현 (X): `딜러가 적립금이 얼마인지 확인할 방법이 없다.`
        - 결함 수정 (O): `필터를 초기화해도 이전 선택값이 남아 목록이 갱신되지 않는다.`
      - `목표` — 무엇을 추가하거나 어떤 상태가 되어야 하는지(추가할 기능 / 기대 결과)
  ...
  "content": [
    {
      "type": "heading",
      "attrs": { "level": 3 },
      "content": [{ "type": "text", "text": "배경·문제" }]
    },
    {
      "type": "paragraph",
      "content": [{ "type": "text", "text": "이 작업이 필요해진 경위 (결함이 확인된 경우에만 문제로 서술)" }]
    },
    {
      "type": "heading",
      "attrs": { "level": 3 },
    ...
    },
    {
      ...
      "content": [
        {
          "type": "listItem",
          "content": [{ "type": "paragraph", "content": [{ "type": "text", "text": "무엇을 추가/달성해야 하는지" }] }]
        }
      ]
    }
  ]
