...
Wave 레이어 그래프·임계 경로·이상 징후를 한 번에 낸다 (이상 징후는 있을 때만 나온다). **이슈를 하나씩 조회해 손으로 그래프를 맞추지 않는다** — 그렇게 하면 12건짜리 에픽에 14회 호출과 12만 토큰이 든다(실측).
...
3. **결론 2~3줄** — 지금 착수할 것 하나(여러 개면 임계 경로 위의 것)와 병렬 후보. 이슈 키는 `https://prndcompany.atlassian.net/browse/KEY` 링크로 건다

2·3단계에서 확인한 근거는 결론에 한 줄로 녹인다. 조사 과정을 나열하지 않는다. 그래프 앞에 서론을 쓰지 않는다.
