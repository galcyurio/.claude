...
관계표·착수 가능·대기·임계 경로·이상 징후를 한 번에 낸다. **이슈를 하나씩 조회해 손으로 그래프를 맞추지 않는다** — 그렇게 하면 12건짜리 에픽에 14회 호출과 12만 토큰이 든다(실측).
...
3. **관계표 또는 트리** — 이슈 키는 `https://prndcompany.atlassian.net/browse/KEY` 링크로 건다
4. **이상 징후** — 스크립트가 짚은 것
