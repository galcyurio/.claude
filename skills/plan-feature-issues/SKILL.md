...
- **미결/블로커 표시**: 답변 대기 중인 결정(예: 구간 경계, 0원 포함 여부)은 작업 description에 메모로 남긴다. 다만 **그것이 오지 않으면 착수할 수 없는 것**은 메모가 아니라 7-4의 외부 의존으로 건다 — 메모는 그래프가 읽지 않아 막힌 이슈가 착수 가능으로 나온다.
...
- **외부 블로커**: 서버 API 전달·디자인 에셋 전달·기획 결정처럼 Jira 이슈가 아닌 블로커는 이슈 간 링크로 표현할 수 없다. 7-4로 건다.
- ⛔ "같이 하면 편하다", "이 순서가 자연스럽다" 정도로는 링크하지 않는다. 링크가 많아지면 순서 정보가 아니라 노이즈가 된다.

**7-2. 사용자 확인 (필수) — 확인과 링크 생성을 한 턴에서 끝낸다**

의존성 표를 응답 본문에 쓰고 "다음 턴에 묻겠다"로 끝내지 않는다. **표와 Wave를 `AskUserQuestion` 옵션의 `preview`에 담아 같은 턴에 묻는다.**

`preview`는 질문 UI 옆에 나란히 렌더되므로 본문처럼 가려지지 않는다. "재진술은 질문과 같은 턴에 두지 않는다"는 별도 규칙이 있더라도, 그 규칙이 막는 것은 **본문**이 가려지는 경우이지 `preview`가 아니다. 표를 preview에 넣으면 턴을 나눌 이유가 사라진다.

- 질문: `블로커 링크 N건과 외부 의존 M건을 이대로 걸까요?`
- 옵션: `이대로 생성 (Recommended)` / `일부 수정` / `생략`
- `preview`: `선행 → 후행 + 근거 1줄` 전체와 Wave 구성, 그리고 외부 의존 목록(`[출처] 무엇을 · 어느 이슈가 · 근거 URL`)
- 답을 받으면 **그 자리에서 7-3과 7-4를 실행한다.** 다시 확인받지 않는다.
...
**7-4. 외부 의존 — Jira 이슈가 아닌 블로커**

서버 API 전달·디자인 에셋 전달·기획 결정처럼 **Jira 이슈로 존재하지 않는 블로커**는 의존하는 이슈마다 remote link 로 건다. `jira-deps.py` 가 이 링크를 읽으므로 그래프에서 blocker 로 계산된다.

```bash
python3 ~/.claude/skills/order-jira-issues/extdep.py add \
  --issues HDA-22886,HDA-22891 \
  --source server --slug reward-api \
  --title "적립금 잔액·내역 조회 API 전달" \
  --url "https://prnd.slack.com/archives/C123/p456" \
  --summary "잔액·내역 조회 엔드포인트 전달 대기"
```

- `--source` 는 `server`·`design`·`plan`·`other` 중 하나다. 제목 라벨(`[서버]`)·아이콘·`⛔` 접두사는 스크립트가 붙인다.
- `--slug` 는 나중에 이 의존을 지목해 해소할 때 쓰는 이름이다.
- **`--issues` 에는 그 의존을 기다리는 이슈를 전부 적는다.** 하나에만 걸면 나머지가 착수 가능으로 나온다.
- ⛔ **상대 팀 Jira 이슈를 `--url` 로 쓰지 않는다.** 서버에서 개발이 끝난 것과 그것이 클라이언트에 전달된 것은 다른 사건이다. 이슈 상태를 해소 신호로 삼으면 아직 막혀 있는 이슈가 착수 가능으로 올라온다. 근거로는 전달 여부를 확인할 수 있는 자리(요청 스레드·API 문서·시안)를 쓰고, 상대 이슈 키는 `--summary` 에 적는다.
- ⛔ **"있으면 편한 참고 자료"는 걸지 않는다.** 기준은 이슈 간 링크와 같다 — 그것이 오지 않으면 시작하거나 끝낼 수 없을 때만 건다.
- 전달이 오면 `extdep.py resolve --epic <에픽> --slug <slug>` 로 해소한다. 링크는 지우지 않고 제목만 `✅` 로 바뀌므로 무엇을 기다렸는지가 이슈에 남는다.

### 8. 검증 및 착수 순서 보고

**보고는 아래 네 슬롯을 모두 채운다. 하나라도 비어 있으면 7단계가 끝나지 않은 것이므로 8단계가 아니다.**

1. **생성된 이슈** — 키·제목과 parent·assignee 확인 결과. prefix automation은 **비동기**다(생성 후 최대 ~2분). 생성 직후 응답엔 prefix가 없고 잠시 후 붙는다 → 시간차 두고 검증.
2. **링크 검증 결과** — `acli jira workitem view <후행> --fields "key,issuelinks"`로 확인한 `inwardIssue` 출력. 사용자가 `링크 생략`을 골랐다면 "사용자 요청으로 생략"이라고 적는다. **아직 안 걸었다는 말로는 이 슬롯을 채울 수 없다.**
3. **착수 순서** — **blocked by가 없는 이슈 = 지금 착수 가능**. Wave 단위로 정리해 어떤 이슈가 무엇을 기다리는지 함께 적는다.
4. **외부 의존** — 7-4로 건 항목을 `[출처] 무엇을 · 어느 이슈가 기다리는지`로 적는다. 걸 것이 없었으면 "없음"이라고 적는다.
...
| Jira 이슈가 아닌 블로커는 remote link로 | description 메모는 그래프가 읽지 않는다. 막힌 이슈가 착수 가능으로 나온다 |
...
- "의존성 표를 본문에 쓰고 다음 턴에 묻겠다고 예고했다" → 7-2 위반. 표는 `AskUserQuestion`의 `preview`에 넣어 같은 턴에 묻는다. 예고는 다음 턴을 보장하지 않는다.
...
- "서버 API·시안 대기를 description에만 적었다" → 7-4 누락. 그래프가 그 이슈를 착수 가능으로 낸다.
- "상대 팀 Jira 이슈를 외부 의존 URL로 걸었다" → 개발 완료와 전달은 다른 사건이다. 전달을 확인할 수 있는 자리를 URL로 쓴다.
- "같은 의존을 대표 이슈 하나에만 걸었다" → 나머지 이슈가 착수 가능으로 나온다. 기다리는 이슈 전부에 건다.
