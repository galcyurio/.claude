...
- 질문: `블로커 링크 N건을 이대로 걸까요?`
- 옵션: `이대로 링크 생성 (Recommended)` / `일부 수정` / `링크 생략`
- `preview`: `선행 → 후행 + 근거 1줄` 전체와 Wave 구성
- 답을 받으면 **그 자리에서 7-3을 실행한다.** 다시 확인받지 않는다.
...
**보고는 아래 세 슬롯을 모두 채운다. 하나라도 비어 있으면 7단계가 끝나지 않은 것이므로 8단계가 아니다.**
