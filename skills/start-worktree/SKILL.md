...
argument-hint: "<JIRA-KEY> [--base <ref>] [--new] [--no-move]"
...
2. **base 결정** — `mcp__claude_ai_Atlassian__getJiraIssue`로 부모 에픽 키를 얻는다. `git ls-remote --heads origin 'feature-base/<에픽키>*'`에 결과가 있으면 그 전체 이름을 base로 쓰고, 없으면 `develop`을 쓴다. 후보가 둘 이상이면 `AskUserQuestion`으로 묻는다.
...
```
bash ~/.claude/skills/start-worktree/start-worktree.sh <키> --base <base> --branch <branch>
```
...
| base를 직접 지정 | `--base <ref>` (2번 절차를 건너뛴다) |
