...
argument-hint: "[feature-base|HDA-키] [auto: CI 통과 시 approve+머지까지] [--continue]"
---
...
**모드 — 마지막 인자로 선택.** 기본은 CI 결과 보고까지, `auto`는 실제 머지 완료까지 자동:

| 모드 | 명령 | 어디까지 |
|------|------|----------|
| 기본 | `merge-develop.sh HDA-xxxx` | PR 감지 → CI 대기 → **보고**. approve·머지는 사람이 |
| **`auto`** | `merge-develop.sh HDA-xxxx auto` | 위 + CI 통과 시 **approve → auto-merge → 실제 머지 완료까지** 자동 |
