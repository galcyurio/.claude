...
- `<difit-command> <target> --comment '...' --comment '...' --clean --no-open --port <N>`를 **하니스 백그라운드(run_in_background)로 실행**한다 (계약의 "실행"·"수명" — `--clean`으로 이전 세션 코멘트 격리, `--background`·`--keep-alive` 미사용, 브라우저 닫힘 시 잡 자연 완료가 6-D 트리거).
