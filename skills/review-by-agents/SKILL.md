...
### 1. `src/foo.ts:42` — {한 문장 요약} 🔴 Critical · 🧠 Logic
...
### 2. `src/auth.ts:18` — {한 문장 요약} 🟡 Warning · 🛡️ Security
...
### 1. `src/Button.tsx:30` — 버튼 높이 불일치 🟡 Warning · 🎨 Design
...
- **디자인 검토 섹션**: Designer 결과만 포함한다. Figma 링크가 없어 Designer를 스폰하지 않은 경우 섹션 전체를 생략한다. Designer가 스폰됐지만 이슈가 0건이면 섹션 대신 "이상 없음"에 `🎨 Design`만 표시한다.
- 각 이슈는 표 없이 헤딩 한 줄 + 본문 형식으로 구성한다 (위 마크다운 예시 참고). 헤딩에는 순번, 백틱으로 감싼 `파일:라인`, ` — `, 한 문장 요약, ` `, 심각도 이모지+이름, ` · `, 관점 이모지+이름이 들어간다.
...
- 코드 예시가 필요한 경우 제안 줄 다음에 빈 줄 1개를 두고 언어 지정 코드블록을 배치한다.
- 이슈 사이에는 빈 줄 1개만 둔다. 각 섹션 헤딩 바로 위에는 Unicode 구분선 `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━` 한 줄을 둔다 (첫 섹션 포함, 단 `# 코드 리뷰 결과` 상단에는 두지 않는다).
- **섹션 헤딩 prefix 이모지**: `## 📌 핵심 이슈`, `## 🎨 디자인 검토`, `## ✅ 이상 없음`.
- **이모지 매핑** (이슈 헤딩 인라인용):
