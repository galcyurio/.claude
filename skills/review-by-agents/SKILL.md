...
**기본은 3-A Workflow fan-out이다.** 아래 두 트랙 중 하나 — **Track A(기계적 제거/리네임)** 또는 **Track B(additive DTO/매퍼)** — 의 **공통 조건 + 해당 트랙 적용 조건 + 해당 트랙 강제 게이트를 전부** 충족할 때만 fan-out을 생략하고 오케스트레이터가 직접 검증으로 대체할 수 있다. 어느 트랙에도 깔끔히 맞지 않거나 판단이 흔들리면 정규 Workflow(3-A)로 진행한다.

**fast-path가 대체하는 것은 3-A 코드 리뷰 fan-out뿐이다.** 2단계 외부 링크/컨텍스트 수집(Slack API 스펙 스레드·Notion API 명세 등 서버-클라 계약의 원천)은 **생략 대상이 아니며 반드시 수행한다** — 두 트랙 모두 semantic 검증(계약상 올바른지)의 원천 출처이며, 특히 Track B는 이 계약이 **검증의 1차 기준**이다. 3-B 디자인 리뷰도 Figma 링크 확보 시 그대로 진행한다. (2단계의 feature-memory 조회는 페이지가 없으면 조용히 skip해도 되지만, 외부 링크 수집은 skip하지 않는다.)

**공통 조건 (두 트랙 모두 AND)**:

- 보안 표면이 사실상 0 — auth/인가·crypto·injection·secret·파일 IO·신뢰 못 할 외부 입력 파싱이 없다. 있으면 fast-path 무효(Security 포함 정규 3-A).
- 레포/PR head 조회가 가능하다(불가 시 무효).

**Track A — 기계적 제거/리네임 · 적용 조건 (전부 AND)**:
...
**Track B — additive DTO/매퍼 · 적용 조건 (전부 AND)**:
...
- **`--clean`을 반드시 붙인다** — difit는 코멘트를 브라우저 localStorage(origin=`localhost:<port>`)에 영속하고 로드 시 이전 세션·타 PR 코멘트를 복원하므로, 없으면 잔존 코멘트가 이번 리뷰의 게이트(6-B)와 6-D 회수를 오염시킨다. `--clean`을 붙이면 클라이언트가 로드 시 localStorage를 비워 **덤프가 프리로드 + 사용자 추가분만** 담게 된다(상세: 계약의 "코멘트 영속과 세션 격리").
...
- 모드별 런치 형태로 **하니스 백그라운드(run_in_background)로 실행**한다 (계약의 "실행"·"수명" — `--clean`으로 이전 세션 코멘트 격리, `--background`·`--keep-alive` 미사용, 브라우저 닫힘 시 잡 자연 완료가 6-D 트리거):
  - **현재 변경사항 모드**: `<difit-command> HEAD <base> --merge-base --comment '...' --comment '...' --clean --no-open --port <N>`
  - **PR 모드**: `gh pr diff <PR-URL> | <difit-command> --comment '...' --comment '...' --clean --no-open --port <N>` — diff가 stdin으로 들어오므로 target 인자·`--merge-base`를 붙이지 않는다.
...
- **저위험 fast-path (2트랙)**: **Track A**(기계적 제거/리네임 — 새 로직 없음 + PR head grep 잔존 참조 0건 + 동작 지점 ≤3 각각 직접 Read) 또는 **Track B**(additive DTO/매퍼 — 계약 1:1 대조 + 매퍼 왕복 대칭 확인 + 새 sealed subtype의 exhaustive `when` 커버 grep)의 공통 조건(보안 표면 0 등)+트랙 조건+게이트를 **전부** 충족할 때만 3-A Workflow fan-out을 생략할 수 있다. 미충족·모호하면 정규 Workflow. 상세는 3-0.
- **이슈 제한**:
