...
difit를 띄운 경우, 사용자는 difit에서 직접 리뷰하며 내 코멘트에 답글을 달거나 새 코멘트를 남길 수 있다. 사용자가 완료를 알리면 `difit comment get`으로 그 답변을 회수해 정리 보고한다(6-D). **회수는 읽기 전용**이며, 회수 전에 서버를 종료하면 답변이 복구 불가로 소실된다.
...
**difit 명령(`<difit-command>`) 결정**:

- `command -v difit` 성공 → `difit`
- 실패 → `npx difit`
- 샌드박스에서 네트워크 권한이 없어 `npx difit` 실행이 막히면, 권한 상승을 요청하고 사용자 승인 후 실행한다.
- 모든 difit 호출에 `--no-open --keep-alive`를 기본으로 붙인다(브라우저 자동 실행 회피, 서버 상시 유지). 포트는 `--port <N>`으로 명시해 URL을 확정하고 다른 세션 difit와의 충돌을 피한다(점유 시 difit가 자동으로 다음 포트로 이동하므로 실제 바인딩 포트를 런치 후 확인한다).
...
PR 모드의 GitHub fetch나 `npx difit`는 네트워크가 필요하므로 샌드박스에서는 위 권한 규칙을 따른다.
...
- **시크릿 금지**: 토큰·비밀번호·API 키·개인키·PII 등 자격증명류를 `--comment` body나 명령줄 인자에 절대 복사하지 않는다.

**런치**:

- `<difit-command> <target> --comment '...' --comment '...' --no-open --keep-alive --port <N>`를 **하니스 백그라운드(run_in_background)로 실행**한다. difit 자체 `--background` 플래그는 쓰지 않는다 — 래퍼가 SIGTERM(exit 143)으로 죽을 때 분리된 서버 자식까지 동반 종료돼 URL이 무효화된다. 하니스 백그라운드 + `--keep-alive`라야 턴을 넘어 서버가 유지된다.
...
- **baseline 캡처**: `200` 확인 직후 `difit comment get --port <N> --format json`을 1회 실행해 반환된 모든 메시지 `id`를 baseline 집합으로 기록한다. 이는 방금 내가 프리로드한 코멘트의 id이며, 6-D에서 사용자가 추가한 답변을 가려내는 기준이다(프리로드 0건이면 baseline은 빈 집합).
...
리뷰·답글 작성을 마치면 알려주세요 — difit 코멘트를 회수해 정리합니다.
````
...
- **readback 진입 안내**: `difit: {URL}` 다음 줄에 사용자에게 리뷰·답글을 마치면 알려달라는 안내를 한 줄 둔다. 안내 후 **턴을 종료하고** 사용자 완료 신호를 기다린다(6-D). 서버는 `--keep-alive` + 하니스 백그라운드로 턴을 넘어 유지되므로, 회수 전까지 절대 종료하지 않는다.
...
difit 코멘트는 **서버 메모리에만** 보관되고 디스크에 영속화되지 않는다. 따라서 **서버를 종료하기 전에 반드시 `difit comment get`으로 회수**해야 한다. 종료(kill/SIGTERM) 후에는 사용자가 남긴 답변이 복구 불가능하게 사라진다. **회수가 항상 종료보다 먼저다.**

**절차**:

1. **완료 대기** — 6-B 출력에서 사용자에게 리뷰 완료를 알려달라고 안내한 뒤 **턴을 종료한다**. 사용자가 완료를 알리는 메시지(예: "완료", "done", "확인했어")로 돌아올 때까지 서버를 살린 채 기다린다. 사용자 응답 없이 먼저 회수·종료하지 않는다.
2. **회수** — 완료 신호를 받으면 `difit comment get --port <N> --format json`을 실행한다. 반환 형식:
   ```json
   {"version":<int>,"threads":[{"id":"…","filePath":"…","position":{"side":"new","line":<int>},"messages":[{"id":"…","body":"…","createdAt":"…","updatedAt":"…"}]}]}
   ```
3. **사용자 추가분 식별** — 6-A에서 기록한 baseline 메시지 id 집합 `B`와 대조한다. 각 thread의 각 message에 대해:
   - `message.id ∉ B` → **사용자가 추가한 코멘트/답글**. thread.id가 baseline에 없으면 신규 코멘트, 기존 thread에 붙은 새 message면 내 코멘트에 대한 답글.
   - `message.id ∈ B`이지만 `updatedAt`이 baseline보다 나중 → 사용자가 내 코멘트를 수정한 것으로 간주해 포함.
   - 그 외(B에 있고 변경 없음)는 내 프리로드 그대로이므로 **보고에서 제외**한다.
...
5. **서버 종료** — 보고를 마친 뒤 `lsof -ti tcp:<N> | xargs kill`로 difit 서버를 종료하고 종료됨을 한 줄로 알린다.
...
- 사용자 추가분이 0건이면 `## 💬 difit 답변 회수` 아래 `남긴 답변 없음` 한 줄만 출력하고 서버를 종료한다.
- `(답글)`/`(신규 코멘트)` 구분은 3단계 식별 결과를 따른다. 내 원본 코멘트 맥락이 필요하면 같은 thread의 baseline 메시지 body를 함께 인용한다.
- 사용자가 difit 탭을 닫은 뒤 완료를 알려도 서버 메모리에 코멘트가 남아 있으므로 회수에 영향 없다. 단 사용자가 서버까지 직접 종료했다면 회수 불가이며, 그 경우 답변을 채팅으로 다시 받아야 한다.
...
- **difit 서버 수명·답변 회수**: difit 코멘트는 서버 메모리에만 있고 디스크에 영속화되지 않는다. **6-D readback로 `difit comment get` 회수를 마치기 전에는 서버를 절대 종료(kill/SIGTERM)하지 않는다** — 먼저 종료하면 사용자가 남긴 답변이 복구 불가로 소실된다. 회수가 항상 종료보다 먼저다.
