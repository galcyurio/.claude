...
- 모든 difit 호출에 `--no-open`을 기본으로 붙인다(브라우저 자동 실행 회피). **`--keep-alive`는 붙이지 않는다** — 이게 없어야 사용자가 브라우저를 닫을 때 difit가 스스로 종료되고, 그 종료가 "리뷰 끝" 신호(6-D)가 된다. 포트는 `--port <N>`으로 명시해 URL을 확정하고 다른 세션 difit와의 충돌을 피한다(점유 시 difit가 자동으로 다음 포트로 이동하므로 실제 바인딩 포트를 런치 후 확인한다).
...
- `<difit-command> <target> --comment '...' --comment '...' --no-open --port <N>`를 **하니스 백그라운드(run_in_background)로 실행**한다. difit 자체 `--background` 플래그는 쓰지 않는다 — 평범한 명령을 하니스 백그라운드 잡으로 띄워야 서버가 턴을 넘어 유지되다가, 사용자가 브라우저를 닫으면 잡이 **자연 완료**되어 나를 깨운다(6-D 트리거).
...
difit는 `--keep-alive` 없이 하니스 백그라운드 잡으로 떠 있다가 **사용자가 브라우저를 닫으면 스스로 종료**한다(client disconnect → shutdown). 이 잡 완료가 readback 트리거다 — 사용자의 완료 메시지를 기다리지 않는다. difit는 종료 직전 세션의 모든 코멘트를 stdout에 덤프하므로, **잡 완료 출력(stdout)을 그대로 읽어 회수**한다. 별도 `difit comment get` 호출이나 서버 kill이 필요 없다.
...
2. **덤프 파싱** — 잡 완료 시 stdout의 `📝 Comments from review session:` 블록을 읽는다. 각 thread는 `==================================================` 구분선 사이에 `<file>:L<line>`(또는 `L<start>-L<end>`) + 첫 메시지 본문으로 나오고, 사용자 답글은 그 아래 `Reply <N> (<author>)` 라벨 + 본문으로 붙는다. 마지막 줄은 `Total comments: N`. 블록이 없거나 `Total comments: 0`이면 남긴 코멘트 없음으로 본다.
...
5. **kill 금지** — difit는 이미 자가 종료됐으므로 종료 명령이 필요 없다. **어떤 경우에도 difit에 kill/pkill/`lsof … | xargs kill`을 실행하지 않는다** — difit는 브라우저를 자식 프로세스로 띄우므로 kill하면 사용자 브라우저까지 함께 꺼질 수 있다.
...
- **difit 수명·회수**: difit는 `--keep-alive` 없이 띄워 **사용자가 브라우저를 닫으면 스스로 종료**한다(하니스 백그라운드 잡 완료 = 6-D 회수 트리거). 종료 직전 세션의 모든 코멘트를 stdout에 덤프하므로 그 출력으로 회수한다. **difit를 절대 kill(kill/pkill/`lsof … | xargs kill`)하지 않는다** — difit는 브라우저를 자식 프로세스로 띄우므로 kill하면 사용자 브라우저까지 종료될 수 있고, 자가 종료가 정상 경로다.
