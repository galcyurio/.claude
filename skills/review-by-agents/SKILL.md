...
디자인 변경이 **없으면** 이 fallback을 건너뛴다. 디자인 변경이 **있는데 PR 본문에서 Figma 링크를 확보하지 못했으면**, 아래 순서로 탐색하고 **하나라도 찾으면 즉시 중단**한다:

1. **Jira 이슈 본문** — 이슈 ID는 PR 제목·본문 → 브랜치명(`git rev-parse --abbrev-ref HEAD`) → 최근 커밋 메시지에서 `{PROJECT}-{번호}`(예 `HDA-21304`) 패턴으로 추출한다. `mcp__claude_ai_Atlassian__getJiraIssue(issueIdOrKey, fields=["description","comment","parent"], responseContentFormat="markdown")`의 description·comment와 `mcp__claude_ai_Atlassian__getJiraIssueRemoteIssueLinks`의 웹 링크에서 `figma.com/design/`·`figma.com/board/` URL을 찾는다. 이슈가 subtask면 `parent` 이슈도 같은 방식으로 확인한다.
2. **feature-memory** — feature-memory는 Jira Epic 단위 현황을 Notion 페이지로 관리한다. epic_key(1번 이슈 ID, subtask면 그 parent epic)로 `mcp__claude_ai_Notion__notion-search` → 페이지가 있으면 `mcp__claude_ai_Notion__notion-fetch(page_id)`로 본문을 받아 `## 한눈에 보기`의 `[Figma](url)` 또는 `## 📚 Reference`의 Figma frame 링크에서 URL을 추출한다. 검색 결과가 없으면(= feature-memory 미등록) 건너뛴다.
...
- Figma 링크 확보(출처 무관) → Designer 스폰 대상으로 표시한다.
