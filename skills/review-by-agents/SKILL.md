...
| 모드 | difit target |
|------|--------------|
| PR 모드 | `<difit-command> <PR-URL>` — difit가 GitHub PR diff를 직접 가져온다 |
...
- `<difit-command> <target> --comment '...' --comment '...'`를 **백그라운드로 실행**하고, 출력된 difit URL을 회수한다.
...
- 검증(item 1)·schema 강제(item 4)는 best-effort:
  ...
  - schema: 에이전트 프롬프트에 출력 JSON 계약을 명시하고, 결과가 malformed면 해당 에이전트를 1회 재프롬프트한다.
