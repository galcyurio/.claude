...
**3-0. 기계적 변경 fast-path 판정 (3-A 진입 전 게이트)**

**기본은 3-A Workflow fan-out이다.** 아래 **적용 조건을 전부** 충족할 때만 fan-out을 생략하고 오케스트레이터가 직접 검증으로 대체할 수 있다. 하나라도 불충족이거나 모호하면 정규 Workflow(3-A)로 진행한다.

**fast-path가 대체하는 것은 3-A 코드 리뷰 fan-out뿐이다.** 2단계 외부 링크/컨텍스트 수집(Slack API 스펙 스레드 등 서버-클라 계약의 원천)은 **생략 대상이 아니며 반드시 수행한다** — semantic 검증(리네임/대체가 계약상 올바른지)의 원천 출처다. 3-B 디자인 리뷰도 Figma 링크 확보 시 그대로 진행한다. (2단계의 feature-memory 조회는 페이지가 없으면 조용히 skip해도 되지만, 외부 링크 수집은 skip하지 않는다.)
...
**강제 게이트 (스킵의 대가 — 하나라도 수행 못 하면 fast-path 무효 → 3-A로)**:
...
- 레포가 로컬에 없거나 grep이 불가능하면 fast-path 무효.

**"trivial해 보인다"만으로는 부족하다** — 위 조건이 명시적으로 충족돼야 한다. 판단이 조금이라도 흔들리면 3-A.

**fast-path 출력 플러밍**: 직접 검증에서 발견한 이슈를 `codeIssues`와 동일한 형태(`file`·`line`·`severity`·`perspective`·`issue`·`problem_code`·`suggestion`)로 정리해 4단계에 넘긴다. 선별(검증된 Critical 전량 + non-critical 최대 5건)·5단계 판정·6단계 difit 게이트는 그대로 적용한다. 6-C/6-B 요약 라인의 에이전트 표기는 `직접 검토(fast-path: 기계적 변경, PR브랜치 grep 검증)`로 한다. (구조는 "Workflow 실패 시 폴백"과 유사하나, fast-path는 애초에 fan-out을 스폰하지 않는 점이 다르다.) 디자인 리뷰(3-B)는 fast-path와 무관하게 Figma 링크 확보 시 정상 진행한다.
...
- **기계적 변경 fast-path**: 필드 제거/리네임 등 순수 기계적 diff는 3-0 조건(널 처리·피연산자 리네임 외 새 로직 없음 + PR head grep 잔존 참조 0건 + 동작 지점 ≤3 각각 직접 Read)을 **전부** 충족할 때만 3-A Workflow fan-out을 생략할 수 있다. 조건 미충족·모호하면 정규 Workflow. 상세는 3-0.
- **이슈 제한**:
