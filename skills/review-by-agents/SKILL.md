...
Workflow는 `{ targetDesc, codeIssues, verifyStats }`를 반환한다. `codeIssues`는 Critical·머지차단 Warning 전량 + 나머지 non-critical 최대 5건으로 이미 선별·정렬된 상태이며, Critical·머지차단 finding은 검증자 2명으로 교차검증을 거쳐 `verifyNote`가 부착돼 있다(상세: 4·5단계, `review-code-workflow.js`).
...
- **코드 이슈**: Workflow가 반환한 `codeIssues`. 이미 중복 제거·선별·정렬이 끝난 상태다. 추가 가공하지 않는다.
  - Workflow 내부 처리(참고): ① 같은 `file:line`은 높은 severity 유지 ② **Critical·머지차단 Warning 전량 보존(cap 없음)** + 나머지 warning/info만 최대 5건 ③ severity 내림차순 → `file` → `line` 오름차순 정렬. Critical·머지차단 finding은 교차검증으로 `verifyNote`가 부착되며, 강등된 finding은 삭제하지 않고 낮춰진 severity로 남는다.
...
- 선별은 **검증된 Critical·머지차단 Warning 전량 보존 + 나머지 non-critical 최대 5건** 규칙을 그대로 적용한다.
