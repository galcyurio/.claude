...
- 스킬은 오케스트레이터(메인 스레드)에서 실행되므로 에이전트를 스폰할 수 있다 — 단, 스킬이 위임한 서브에이전트는 다시 스폰할 수 없다
...
- 스킬이 **서브에이전트에게 넘기는 프롬프트** 안에서 또 다른 에이전트 스폰 또는 `Agent` 도구 사용을 지시 (중첩 스폰 불가) → 심각
...
#### 검사 10: 스킬 ↔ orchestration 위임 정합성

orchestration.md "스킬 실행 중 오케스트레이션"은 코드 수정 단계에서 라우팅(위임) 규칙을 따르라고 명시한다.

- 스킬 워크플로우가 2개 이상 파일 생성/수정을 위임·서브에이전트 언급 없이 직접 수행하도록 지시하면 → 경고 (orchestration.md 라우팅과 불일치)
- 단일 파일 수정이거나, 명시적으로 Junior/sonnet 서브에이전트에 위임하는 스킬은 통과
