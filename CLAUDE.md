...
- brainstorming 대화가 끝나면 의도 재진술이나 추가 승인 질문 없이 곧바로 spec을 작성한 뒤 보고한다. 두 단계 사이에서 멈추지 않는다.
- 방향 확인은 brainstorming 대화에서 이미 끝났다. 승인을 두 번 받지 않는다.
- 보고에는 spec 절대경로를 별도 라인으로 넣고 리뷰를 요청한다. superpowers `brainstorming`의 spec 리뷰 게이트는 이 보고 시점으로 옮긴다.
- 수정 요청을 받으면 spec을 고친 뒤 다시 보고한다.
