...
- 가능한 한 기획자, 유저의 관점에서 커밋 메시지를 작성한다.
  ...
  - Good: `HDA-126 feat: 알림 모두보기 클릭 시 가격 알림 화면으로 진입한다`
  - Bad: `HDA-126 feat: HomeViewModel에 onNotificationSeeAllClick 핸들러를 추가한다`
  - 내부 리팩토링/구조 변경에서만 클래스/함수명을 직접 언급한다.
- Claude Code 기본 Co-Authored-By trailer를 추가한다.

# 커밋 단위

> 커밋 메시지 형식 자체는 위 `# Git commit message 규칙`을 따른다. 이 절은 **단위 분리**와 **메시지 표현 패턴**을 보완한다.
...
- PR 생성(push) 전에는 기존 커밋에 대한 fixup/squash/amend, interactive rebase를 **명시적 지시 없이도 자유롭게** 수행할 수 있다. 히스토리가 아직 공유되지 않았기 때문이다.
- 단, **무조건 기존 커밋에 흡수하지 않는다.** 새 커밋으로 나누는 것이 적절한 변경은 나눈다.
  - **같은 논리 단위의 보완**(직전 커밋의 누락/오타/스타일 수정)이면 해당 커밋에 fixup으로 흡수한다.
  - **별개의 논리적 변경**이면 `### 한 커밋 = 하나의 논리적 변경` 원칙에 따라 새 커밋으로 분리한다.
- 판단 기준: "이 변경이 기존 커밋 메시지로 설명되는가?" → 예면 흡수, 아니면 새 커밋.

## PR 생성 이후 커밋 전략

- PR을 만든 이후의 코드 수정은 **기본적으로 새 커밋**으로 쌓는다.
- 기존 커밋에 대한 fixup/squash/amend, interactive rebase는 **사용자가 명시적으로 지시한 경우에만** 수행한다. (예: `rebase-fix` 스킬 호출, "fixup으로", "기존 커밋에 합쳐줘")
- 명시적 지시가 없으면 리뷰/디자인 반영도 새로운 커밋으로 분리한다.
...
- 답글은 스레드 안에 남긴다 (`gh api repos/{owner}/{repo}/pulls/{pr}/comments/{id}/replies`). 최상위 PR 코멘트로 달지 않는다.
- 답글에서 커밋을 가리킬 때는 **커밋 메시지를 링크 텍스트로 쓴 마크다운 링크**로 남긴다. 해시 문자열을 그대로 적지 않는다. 링크 텍스트에서 이슈 prefix(`HDA-12345`)는 뺀다.

  ```markdown
  [fix: 상위 선택값 없이 단계를 열면 예외로 남긴다](https://github.com/{owner}/{repo}/commit/{sha})
  ```

  커밋이 여러 개면 링크를 줄바꿈으로 나열한다.
